...
SketchTest 是面向研发、测试和平台工程团队的 REST API 自动化测试平台。平台以 OpenAPI、RAML 和 Git 仓库为测试资产来源，通过规则与 AI 生成可审核的测试用例，并使用面向 API 场景的流程编排能力执行多接口业务链路。每次执行形成包含来源版本、请求响应、断言、环境、代码提交和链路追踪信息的不可变证据记录，用于持续回归、发布门禁和缺陷定位。
